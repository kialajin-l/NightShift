...
export class ConversationLogParser {
    ...
    calculateStatistics(log) {
        ...
        const totalCodeBlocks = messages.reduce((sum, msg) => sum + (msg.metadata?.codeBlocks?.length || 0), 0);
        const totalErrors = messages.reduce((sum, msg) => sum + (msg.metadata?.errors?.length || 0), 0);
        const totalToolCalls = messages.reduce((sum, msg) => sum + (msg.metadata?.toolCalls?.length || 0), 0);
    ...
    calculateSentimentScore(messages) {
        ...
        messages.forEach(message => {
            ...
            // 检查积极关键词
            positiveCount += sentimentKeywords.positive.filter(keyword => content.includes(keyword)).length;
            // 检查消极关键词
            negativeCount += sentimentKeywords.negative.filter(keyword => content.includes(keyword)).length;
        });
     ...
     * 计算复杂度分数
     */
    calculateComplexityScore(messages) {
        ...
        messages.forEach(message => {
            ...
            if (codeBlocks.length > 0) {
                const totalCodeLength = codeBlocks.reduce((sum, block) => sum + block.code.length, 0);
     ...
     * 检测重复指令
     */
    detectRepeatedInstructions(messages) {
        const userMessages = messages.filter(m => m.role === 'user');
     ...
     * 检测用户偏好
     */
    detectUserPreferences(messages) {
        const preferences = [];
        const userMessages = messages.filter(m => m.role === 'user');
        userMessages.forEach(message => {
            const content = message.content.toLowerCase();
     ...
     * 检测命名模式
     */
    detectNamingPatterns(codeBlocks) {
        const patterns = [];
        // 检测变量命名模式
     ...
     * 分析变量命名
     */
    analyzeVariableNaming(codeBlocks) {
        const patterns = [];
     ...
     * 分析函数命名
     */
    analyzeFunctionNaming(codeBlocks) {
        const patterns = [];
        codeBlocks.forEach(block => {
            const code = block.code;
            // 简单的函数命名模式检测
     ...
     * 检测结构模式
     */
    detectStructurePatterns(codeBlocks) {
        const patterns = [];
        // 检测导入模式
     ...
     * 分析导入模式
     */
    analyzeImportPatterns(codeBlocks) {
        const patterns = [];
        codeBlocks.forEach(block => {
            const code = block.code;
            // 检测 import 语句
     ...
     * 检测风格模式
     */
    detectStylePatterns(codeBlocks) {
        const patterns = [];
        // 检测缩进风格
     ...
     * 生成洞察
     */
    generateInsights(log, statistics, patterns) {
     ...
     * 计算技术债务
     */
    calculateTechnicalDebt(patterns, statistics) {
        let debtScore = 0;
        // 基于错误数量
     ...
     * 计算学习进度
     */
    calculateLearningProgress(log, statistics) {
        // 基于消息数量和复杂度
     ...
     * 计算效率
     */
    calculateEfficiency(statistics) {
        // 基于错误率和消息长度
     ...
     * 生成建议
     */
    generateRecommendations(patterns, statistics) {
