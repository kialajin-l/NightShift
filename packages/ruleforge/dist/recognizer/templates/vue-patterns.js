...
export const vuePatterns = [
    {
        ...
        condition: (events) => {
            ...
            const fixEvents = events.filter(e => e.type === 'file_saved' &&
...
const props = defineProps({
  title: {
    ...
    },
    {
        ...
        condition: (events) => {
            // 检查是否有多个文件使用组合式 API
            const setupFiles = events.filter(e => e.type === 'file_saved' &&
                e.content?.includes('<script setup>'));
            const compositionEvents = events.filter(e => e.type === 'file_saved' &&
...
const increment = () => count.value++
    ...
    },
    {
        ...
        condition: (events) => {
            // 检查路由导航相关事件
            const routerEvents = events.filter(e => e.type === 'file_saved' &&
