...
export class PatternRecognizer {
   ...
   * 主入口：从解析后的会话中识别候选模式
   */
  async recognize(session: ParsedSession): Promise<RecognitionResult> {
   ...
   * 过滤排除的文件事件
   */
  private filterEvents(events: ParsedEvent[]): ParsedEvent[] {
  ...
  private analyzeKeywords(
    ...
    for (const event of events) {
      ...
      const keywords = PatternRecognizer.extractKeywords(
        event.content, 
        event.file ? this.getLanguageFromFile(event.file) : 'unknown'
      );
   ...
   * 阶段2：文件类型聚类
   */
  private clusterEvents(events: ParsedEvent[]): EventCluster[] {
    const clusters: EventCluster[] = [];
    const clusterConfig: FilePatternClusterConfig = {
      patterns: [
    ...
    for (const patternConfig of clusterConfig.patterns) {
      const clusterEvents = events.filter(event => 
        event.file && this.matchFilePattern(patternConfig.pattern, event.file)
      );
    ...
    const classifiedFiles = new Set(
      clusters.flatMap(cluster => cluster.events.map(e => e.file).filter(Boolean))
    );
    
    const unclassifiedEvents = events.filter(event => 
      event.file && !classifiedFiles.has(event.file)
    );
  ...
  private matchTemplates(
    ...
    globalKeywordStats: Map<string, KeywordStats>
  ): CandidatePattern[] {
   ...
   * 计算文件模式匹配得分
   */
  private calculateFileScore(filePattern: string, events: ParsedEvent[]): number {
    const matchingEvents = events.filter(event => 
      event.file && this.matchFilePattern(filePattern, event.file)
    );
   ...
   * 生成模式ID
   */
  private generatePatternId(category: string, keywords: string[]): string {
