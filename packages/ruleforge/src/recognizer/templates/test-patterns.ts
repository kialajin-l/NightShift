 ...
 * 测试相关模式模板
 */
export const testPatterns: PatternTemplate[] = [
  {
    ...
    condition: (events) => {
      ...
      const assertionEvents = events.filter(e => 
        e.type === 'file_saved' && 
...
describe('MyComponent', () => {
  ...
  },
  {
    ...
    condition: (events) => {
      // 检查模拟使用模式
      const mockEvents = events.filter(e => 
        e.type === 'file_saved' && 
        (e.content?.includes('mock') || e.content?.includes('spyOn'))
      );
      
      const testRunEvents = events.filter(e => 
        e.type === 'test_run'
      );
...
expect(result.data).toBe('test')`,
  ...
  },
  {
    ...
    condition: (events) => {
      ...
      const teardownEvents = events.filter(e => 
        e.type === 'file_saved' && 
