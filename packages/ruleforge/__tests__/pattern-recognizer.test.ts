...
import { ParsedSession, ParsedEvent } from '../dist/types/pattern.js';
...
describe('PatternRecognizer', () => {
  ...
  let sampleSession: ParsedSession;
...
def get_user(user_id: int):
  ...
  describe('关键词分析', () => {
    it('应该统计关键词频率和上下文', () => {
      ...
      const stats = (recognizer as any).analyzeKeywords(events, {
        minFrequency: 1,
        excludeCommon: true,
  ...
  describe('事件聚类', () => {
    it('应该按文件模式聚类事件', () => {
      const clusters = (recognizer as any).clusterEvents(sampleSession.events);
    ...
    it('应该处理未分类的事件', () => {
      ...
      const clusters = (recognizer as any).clusterEvents(events);
  ...
  describe('置信度计算', () => {
    it('应该正确计算关键词匹配得分', () => {
      ...
      const score = (recognizer as any).calculateKeywordScore(templateKeywords, keywordStats);
  ...
  describe('文件模式匹配', () => {
    it('应该匹配简单的文件模式', () => {
      const match = (recognizer as any).matchFilePattern('**/*.vue', 'src/components/Login.vue');
      expect(match).toBe(true);
      
      const noMatch = (recognizer as any).matchFilePattern('**/*.ts', 'src/components/Login.vue');
      expect(noMatch).toBe(false);
    });

    it('应该匹配目录模式', () => {
      const match = (recognizer as any).matchFilePattern('**/api/*.py', 'src/api/users.py');
      expect(match).toBe(true);
    });

    it('应该处理通用模式', () => {
      const match = (recognizer as any).matchFilePattern('**/*', 'any/file/path');
      expect(match).toBe(true);
    });
  });
  ...
  describe('边界情况处理', () => {
    ...
    it('应该处理只有通用关键词的事件', async () => {
      const genericSession: ParsedSession = {
