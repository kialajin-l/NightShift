...
import { ParsedSession } from '../../dist/types/pattern.js';

/**
 * 生成测试数据：50+ 事件
 */
function generateTestData(): ParsedSession[] {
  const sessions: ParsedSession[] = [];
  
  // 会话 1: Vue 组件开发（高频模式）
  const vueSession: ParsedSession = {
...
export default { props: { value${i}: { type: String, required: true } } }`,
  ...
  const fastapiSession: ParsedSession = {
...
async def endpoint${i}(user = Depends(get_current_user)): return {"data": "protected"}`,
  ...
  // 会话 4: 包含敏感信息
  const sensitiveSession: ParsedSession = {
...
describe('RuleForge 端到端测试', () => {
  ...
  let testSessions: ParsedSession[];
  ...
  describe('置信度评分验证', () => {
    it('应该正确计算置信度评分', async () => {
      ...
      // 分类模式
      const highFrequencyPatterns = patterns.filter(p => 
        p.trigger.frequency >= 5
      );
      const lowFrequencyPatterns = patterns.filter(p => 
        p.trigger.frequency <= 2
      );
