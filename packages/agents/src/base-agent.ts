...
export abstract class BaseAgent implements Agent {
  ...
  private taskHistory: Array<{taskId: string, success: boolean, executionTime: number}> = [];
  ...
  async initialize(): Promise<void> {
    ...
    if (!validation.isValid) {
      throw new AgentError('validation_failed', `技能配置验证失败: ${validation.errors.join(', ')}`);
    }
  ...
  async execute(task: Task): Promise<TaskOutput> {
    ...
    if (this.status.isBusy) {
      throw new AgentError('execution_error' as AgentErrorType, 'Agent 正在执行其他任务');
    }
    ...
    try {
      ...
      if (!taskValidation.isValid) {
        throw new AgentError('invalid_task' as AgentErrorType, `任务验证失败: ${taskValidation.errors.join(', ')}`);
      }
      ...
      throw new AgentError('execution_error', `任务执行失败: ${error instanceof Error ? error.message : String(error)}`, error);
    } finally {
      this.setIdle();
    }
  }
  ...
  protected ensureReady(): void {
    if (!this.isInitialized || !this.status.isReady) {
      throw new AgentError('execution_error', 'Agent 未初始化或未就绪');
    }
  }
  ...
  protected applyModelSelectionRules(task: Task): string | null {
    for (const rule of this.rules) {
      if (this.ruleMatches(rule, task)) {
        ...
        if (modelAction) {
          return modelAction.parameters['model'] as string; // 使用parameters访问索引签名属性
        }
      }
    }
    return null;
   ...
   * 执行单个技能
   */
  protected async executeSkill(skill: Skill, input: SkillInput, context: SkillContext): Promise<SkillOutput> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (!validation.isValid) {
        throw new AgentError('validation_failed', `技能验证失败: ${validation.errors.join(', ')}`);
      }
      ...
      throw new AgentError('execution_error', `技能 ${skill.name} 执行失败: ${error instanceof Error ? error.message : String(error)}`, error);
    }
  }
