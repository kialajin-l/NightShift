 ...
 * 文件锁项
 */
interface FileLockItem extends FileLockInfo {
  timeoutId: NodeJS.Timeout; // 超时计时器
  renewCount: number;         // 续约次数
}

/**
 * 文件锁管理器实现
 */
export class DistributedFileLockManager implements FileLockManager {
  private locks: Map<string, FileLockItem> = new Map();
  private cleanupInterval: NodeJS.Timeout | null = null;
  private config: CommunicationConfig['fileLock'];
   ...
   * 获取文件锁
   */
  async acquireLock(filePath: string, owner: string, timeout?: number): Promise<FileLockInfo> {
   ...
   * 释放文件锁
   */
  async releaseLock(filePath: string, owner: string): Promise<boolean> {
    const normalizedPath = this.normalizePath(filePath);
   ...
   * 续约文件锁
   */
  private renewLock(filePath: string, owner: string, timeout: number): FileLockInfo {
    const normalizedPath = this.normalizePath(filePath);
    const lock = this.locks.get(normalizedPath);
   ...
   * 创建超时计时器
   */
  private createTimeout(filePath: string, timeout: number): NodeJS.Timeout {
    return setTimeout(() => {
      const lock = this.locks.get(filePath);
      if (lock) {
  ...
  getAllActiveLocks(): Map<string, FileLockInfo> {
    const activeLocks = new Map<string, FileLockInfo>();
