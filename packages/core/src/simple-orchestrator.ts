...
import { WorkflowResult, Task } from './types';
...
export class SimpleNightShiftOrchestrator {
   ...
   * 执行任务
   */
  private async executeTasks(tasks: Task[], dependencies: Map<string, string[]>): Promise<any[]> {
    const results = [];
    ...
    while (completedTasks.size < tasks.length) {
      ...
      const batchResults = await Promise.all(
        batch.map(task => this.executeSingleTask(task))
      );
