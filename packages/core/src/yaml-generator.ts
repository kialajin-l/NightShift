import { RuleCandidate, REPRule, YAMLGeneratorConfig } from './types';
...
export class YAMLGenerator {
  private config: YAMLGeneratorConfig;
   ...
   * 转换为 REP v0.1 规则格式
   */
  private convertToREPRule(candidate: RuleCandidate): REPRule {
   ...
   * 映射建议类型
   */
  private mapSuggestionType(type: string): string {
    const mapping: Record<string, string> = {
   ...
   * 生成默认模板
   */
  private generateDefaultTemplate(candidate: RuleCandidate): string {
   ...
   * 生成代码示例
   */
  private generateCodeExample(candidate: RuleCandidate): string | undefined {
    if (!this.config.includeExamples) return undefined;
   ...
   * 创建代码示例
   */
  private createCodeExample(candidate: RuleCandidate): string {
    switch (candidate.type) {
      case 'keyword':
   ...
   * 创建关键词示例
   */
  private createKeywordExample(candidate: RuleCandidate): string {
...
// 这里可以添加具体的代码实现`;
   ...
   * 创建文件类型示例
   */
  private createFileTypeExample(candidate: RuleCandidate): string {
...
// 这里可以添加具体的文件内容`;
   ...
   * 创建错误示例
   */
  private createErrorExample(candidate: RuleCandidate): string {
...
// 这里可以添加具体的修复代码`;
   ...
   * 创建结构示例
   */
  private createStructureExample(candidate: RuleCandidate): string {
...
// 这里可以展示具体的代码结构`;
   ...
   * 生成建议描述
   */
  private generateSuggestionDescription(candidate: RuleCandidate): string {
    switch (candidate.type) {
      case 'keyword':
   ...
   * 检测支持的语言
   */
  private detectLanguages(candidate: RuleCandidate): string[] {
   ...
   * 检测支持的框架
   */
  private detectFrameworks(candidate: RuleCandidate): string[] {
   ...
   * 检测支持的工具
   */
  private detectTools(candidate: RuleCandidate): string[] {
   ...
   * 验证 REP Schema
   */
  private validateREPSchema(rule: REPRule): void {
    const errors: string[] = [];
    
    // 验证 meta 字段
    if (!rule.meta.id) errors.push('meta.id 不能为空');
    if (!rule.meta.name) errors.push('meta.name 不能为空');
    if (!rule.meta.version) errors.push('meta.version 不能为空');
    
    // 验证 rule 字段
    if (!rule.rule.trigger.type) errors.push('rule.trigger.type 不能为空');
    if (!rule.rule.trigger.pattern) errors.push('rule.trigger.pattern 不能为空');
    if (!rule.rule.suggestion.template) errors.push('rule.suggestion.template 不能为空');
    if (!rule.rule.suggestion.description) errors.push('rule.suggestion.description 不能为空');
...
`;
   ...
   * 生成文件名
   */
  private generateFilename(candidate: RuleCandidate): string {
