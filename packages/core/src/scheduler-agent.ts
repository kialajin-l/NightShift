import { Task, Agent, WorkflowResult } from './types';

/**
 * 调度 Agent - 负责任务拆解和依赖关系管理
 */
export class SchedulerAgent {
  private name = '调度 Agent';
  private role = '任务拆解专家';
  private model = 'glm-5';
  private skills = ['prd_parser', 'dag_generator', 'dependency_analyzer'];

  /**
   * 解析用户需求并生成任务计划
   */
  async parseRequirements(userInput: string): Promise<{
    tasks: Task[];
   ...
   * 提取主要目标
   */
  private extractMainGoal(userInput: string): string {
   ...
   * 从关键词推断功能
   */
  private inferFeaturesFromKeywords(userInput: string): string[] {
    const features: string[] = [];
   ...
   * 提取技术要求
   */
  private extractTechnicalRequirements(userInput: string): string[] {
   ...
   * 确定优先级
   */
  private determinePriority(userInput: string): 'low' | 'medium' | 'high' {
   ...
   * 任务拆解
   */
  private decomposeTasks(analysis: RequirementAnalysis): Task[] {
    const tasks: Task[] = [];
    const now = new Date();
   ...
   * 确定任务类型
   */
  private determineTaskType(feature: string): Task['type'] {
   ...
   * 分配 Agent
   */
  private assignAgent(taskType: Task['type']): string {
    const agentMap: Record<Task['type'], string> = {
   ...
   * 分配模型
   */
  private assignModel(taskType: Task['type']): string {
    const modelMap: Record<Task['type'], string> = {
   ...
   * 生成任务名称
   */
  private generateTaskName(feature: string): string {
   ...
   * 生成任务描述
   */
  private generateTaskDescription(feature: string, mainGoal: string): string {
   ...
   * 估算任务时间
   */
  private estimateTaskTime(feature: string, taskType: Task['type']): number {
   ...
   * 估算复杂度
   */
  private estimateComplexity(feature: string): number {
    ...
    for (const task of tasks) {
      ...
      if (task.type === 'test') {
        const featureTask = tasks.find(t => 
          t.name.includes(task.name.replace('测试', '').replace('功能', ''))
        );
        if (featureTask) {
   ...
   * 生成 DAG（有向无环图）
   */
  private generateDAG(tasks: Task[], dependencies: Map<string, string[]>): TaskDAG {
    const dag: TaskDAG = {
      nodes: tasks,
      edges: [],
      startNodes: [],
    ...
    // 找出起始节点（没有依赖的节点）
    dag.startNodes = tasks.filter(task => 
      !Array.from(dependencies.values()).flat().includes(task.id)
    ).map(task => task.id);
    
    // 找出结束节点（没有被依赖的节点）
    const allDependencies = Array.from(dependencies.keys());
    dag.endNodes = tasks.filter(task => 
      !allDependencies.includes(task.id)
    ).map(task => task.id);
   ...
   * 估算总时间
   */
  private estimateTime(tasks: Task[]): number {
   ...
   * 检查循环依赖
   */
  private checkCyclicDependencies(tasks: Task[], dependencies: Map<string, string[]>): string | null {
   ...
   * 计算并行度
   */
  private calculateParallelism(tasks: Task[], dependencies: Map<string, string[]>): number {
    const executionOrder = this.getExecutionOrder(tasks, dependencies);
   ...
   * 获取执行顺序
   */
  private getExecutionOrder(tasks: Task[], dependencies: Map<string, string[]>): string[][] {
    const order: string[][] = [];
