...
export class MockNightShiftOrchestrator {
   ...
   * 模拟工作流步骤
   */
  private async simulateWorkflowSteps(): Promise<void> {
    const steps = [
   ...
   * 生成模拟结果
   */
  private generateMockResult(userInput: string, executionTime: number): WorkflowResult {
    // 根据用户输入生成不同的任务
   ...
   * 生成模拟任务
   */
  private generateMockTasks(userInput: string): any[] {
   ...
   * 生成模拟文件
   */
  private generateMockFiles(tasks: any[]): GeneratedFile[] {
...
button:disabled {
   ...
   * 生成Python API内容
   */
  private generatePythonAPI(name: string, description: string): string {
    return `"""
${description}
"""
