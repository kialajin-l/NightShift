...
} from '../scheduler/types/task.js';

import {
  AgentTask,
  FrontendTask,
  BackendTask
} from '../../agents/src/types/agent.js';
...
export class NightShiftOrchestrator {
  ...
  private taskPanel: TaskPlanPanel;
   ...
   * 任务分解
   */
  private async decomposeTask(userInput: string): Promise<TaskDecompositionResult> {
   ...
   * Agent 并发执行
   */
  private async executeAgentsConcurrently(tasks: Task[]): Promise<AgentExecutionResult> {
   ...
   * 执行前端 Agent
   */
  private async executeFrontendAgent(tasks: Task[], results: AgentExecutionResult): Promise<void> {
   ...
   * 执行后端 Agent
   */
  private async executeBackendAgent(tasks: Task[], results: AgentExecutionResult): Promise<void> {
   ...
   * 更新进度面板
   */
  private async updateProgressPanel(tasks: Task[]): Promise<void> {
    // 模拟实时进度更新
   ...
   * 提取新规则
   */
  private async extractNewRules(userInput: string, workflowResult: WorkflowResult): Promise<any[]> {
    const sessionData = {
      userInput,
