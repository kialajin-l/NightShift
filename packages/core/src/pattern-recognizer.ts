import { SessionLog, CodePattern, RuleCandidate, PatternRecognitionResult } from './types';
...
export class PatternRecognizer {
  ...
  private minOccurrences = 3; // 最小出现次数
  private minConfidence = 0.7; // 最小置信度
   ...
   * 分析关键词频率
   */
  private analyzeKeywords(): CodePattern[] {
    const keywordFrequency = new Map<string, number>();
    const keywordContexts = new Map<string, string[]>();
   ...
   * 分析文件类型
   */
  private analyzeFileTypes(): CodePattern[] {
    const fileTypeFrequency = new Map<string, number>();
    const fileTypeExamples = new Map<string, string[]>();
   ...
   * 分析错误模式
   */
  private analyzeErrors(): CodePattern[] {
    const errorPatterns: CodePattern[] = [];
   ...
   * 分析代码结构
   */
  private analyzeCodeStructure(): CodePattern[] {
    const structurePatterns: CodePattern[] = [];
    const structureFrequency = new Map<string, number>();
    const structureExamples = new Map<string, string[]>();
   ...
   * 生成规则候选
   */
  private generateRuleCandidates(patterns: CodePattern[]): RuleCandidate[] {
   ...
   * 创建规则候选
   */
  private createRuleCandidate(pattern: CodePattern): RuleCandidate {
    const baseRule = {
   ...
   * 分析文件结构
   */
  private analyzeFileStructure(content: string): string | null {
   ...
   * 生成错误解决方案
   */
  private generateErrorSolutions(errorType: string): string[] {
   ...
   * 生成规则名称
   */
  private generateRuleName(pattern: CodePattern): string {
    switch (pattern.type) {
      case 'keyword':
   ...
   * 生成规则描述
   */
  private generateRuleDescription(pattern: CodePattern): string {
    switch (pattern.type) {
      case 'keyword':
   ...
   * 生成关键词模板
   */
  private generateKeywordTemplate(keyword: string): string {
   ...
   * 生成文件模板
   */
  private generateFileTemplate(fileType: string): string {
   ...
   * 生成结构模板
   */
  private generateStructureTemplate(structureType: string): string {
   ...
   * 计算总体置信度
   */
  private calculateOverallConfidence(candidates: RuleCandidate[]): number {
