 ...
 */
import { RepRuleSchema } from '../types/rep-rule.js';
/**
 * RuleForge YAML 格式化器
 */
export class RuleYamlFormatter {
    ...
    async toYAML(pattern) {
        ...
        if (!repValidation.success) {
            const errors = repValidation.error.errors.map(err => `${err.path.join('.')}: ${err.message}`);
     ...
     * @returns 框架版本约束
     */
    generateFrameworkCompatibility(pattern) {
        const frameworks = {};
        // 根据模式类别推断框架兼容性
