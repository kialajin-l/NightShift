 ...
 */
import { ProtocolErrorCodes } from './types/level1-protocol.js';
/**
 * 消息序列化器实现
 */
export class JsonRpcSerializer {
    ...
    deserialize(data) {
        try {
            // 解析 JSON 字符串
            const message = JSON.parse(data, this.createReviver());
            // 验证消息格式
            const validation = this.validate(message);
            if (!validation.isValid) {
                throw new Error(`消息验证失败: ${validation.errors.join(', ')}`);
     ...
     * 检查消息是否过大
     */
    isMessageTooLarge(message, maxSize) {
        return this.getMessageSize(message) > maxSize;
    }
    /**
     * 分割大消息
     */
    splitLargeMessage(message, maxChunkSize) {
        const chunks = [];
