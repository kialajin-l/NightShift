...
import { MemoryStorage, ConsoleLogger, PriorityQueue, SemaphoreConcurrencyController } from './types/task-manager.js';
/**
 * 任务状态机
 */
class TaskStateMachine {
     ...
     * 执行状态转换
     */
    transition(task, newStatus, data) {
...
class TimeoutDetector {
     ...
     * 设置任务超时检测
     */
    setTimeout(taskId, timeoutMs, callback) {
 ...
 * NightShift 任务管理器实现
 */
export class NightShiftTaskManager extends EventEmitter {
    storage;
    queue;
    ...
    async finalizeTask(taskId, status, data) {
        this.ensureInitialized();
        const task = await this.storage.get(taskId);
        if (!task) {
            throw new Error(`任务不存在: ${taskId}`);
        ...
        // 调用插件钩子
        const hookName = `onTask${status.charAt(0).toUpperCase() + status.slice(1)}`;
