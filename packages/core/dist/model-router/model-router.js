 ...
 */
import { RouterError } from './types/model-router.js';
import { DEFAULT_ROUTER_CONFIG, RouterConfigManager } from './config/router-config.js';
/**
 * 模型路由系统实现
 */
export class SmartModelRouter {
    ...
    trackUsage(model, tokens) {
        ...
        this.usageStats.timeSeries = this.usageStats.timeSeries.filter(ts => ts.timestamp >= twentyFourHoursAgo);
    ...
    healthCheck() {
        ...
        // 检查模型可用性
        const availableModels = this.getAvailableModels().filter(model => this.isModelAvailable(model));
     ...
     * 根据复杂度获取模型类型
     */
    getModelTypeForComplexity(complexity) {
        switch (complexity) {
     ...
     * 检查技术条件
     */
    checkTechnologyCondition(condition, technology) {
        if (!technology)
            return false;
     ...
     * 从规则获取模型
     */
    getModelFromRule(rule) {
        const modelAction = rule.actions.find(action => action.type === 'select_model');
        if (modelAction) {
            return this.availableModels.get(modelAction.value) || null;
        }
        return null;
    }
    /**
     * 智能路由决策
     */
    makeIntelligentRoutingDecision(context, classification) {
     ...
     * 获取适合的模型
     */
    getSuitableModels(context, complexity) {
        ...
        return Array.from(this.availableModels.values()).filter(model => {
            ...
            if (model.type !== targetType)
                return false;
            // 检查能力匹配
            if (!this.modelHasRequiredCapabilities(model, context.task))
                return false;
            // 检查技术栈支持
            if (!this.modelSupportsTechnology(model, context.task.technology))
                return false;
            // 检查性能约束
            if (!this.modelMeetsPerformanceConstraints(model, context.constraints))
                return false;
            return true;
        });
    }
     ...
     * 检查模型能力
     */
    modelHasRequiredCapabilities(model, task) {
        // 根据任务类型确定所需能力
     ...
     * 获取任务所需能力
     */
    getRequiredCapabilitiesForTask(taskType) {
        const capabilityMap = {
     ...
     * 检查技术栈支持
     */
    modelSupportsTechnology(model, technology) {
     ...
     * 检查性能约束
     */
    modelMeetsPerformanceConstraints(model, constraints) {
        // 简化实现
        return true;
    }
    /**
     * 选择最佳模型
     */
    selectBestModel(models, context, estimatedTokens) {
        // 简单的成本效益选择算法
     ...
     * 计算模型评分
     */
    calculateModelScore(model, context, estimatedTokens) {
        let score = 0;
        // 成本因素（越低越好）
     ...
     * 计算能力匹配度
     */
    calculateCapabilityMatch(model, task) {
     ...
     * 生成路由理由
     */
    generateRoutingReasoning(model, context, classification) {
        const reasons = [];
     ...
     * 获取备选模型
     */
    getAlternativeModels(primary, context) {
        const alternatives = Array.from(this.availableModels.values())
            .filter(model => model.name !== primary.name &&
     ...
     * 执行本地程序
     */
    async executeLocalProgram(model, prompt) {
        // 简化实现：模拟本地程序执行
        const startTime = Date.now();
        // 模拟执行延迟
     ...
     * 执行本地模型
     */
    async executeLocalModel(model, prompt) {
        // 简化实现：模拟本地模型调用
        const startTime = Date.now();
        // 模拟API调用延迟
     ...
     * 执行云端模型
     */
    async executeCloudModel(model, prompt) {
        // 简化实现：模拟云端模型调用
        const startTime = Date.now();
        // 模拟网络延迟
     ...
     * 获取默认降级选项
     */
    getDefaultFallbackOptions(primaryModel) {
