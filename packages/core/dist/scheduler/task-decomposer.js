...
class KeywordAnalyzer {
    ...
    assessComplexity(wordCount, keywordDensity, categoryCount) {
...
class DependencyResolver {
    ...
    resolveDependencies(tasks) {
        ...
        for (const task of tasks) {
            ...
            if (task.agent === 'frontend' && task.tags.includes('api_consumer')) {
                const backendTasks = tasks.filter(t => t.agent === 'backend' && t.tags.includes('api_provider'));
            ...
            // 测试依赖功能实现
            if (task.agent === 'test') {
                const functionalTasks = tasks.filter(t => t.agent !== 'test' && !t.tags.includes('infrastructure'));
            ...
            if (task.tags.includes('database')) {
                const backendTasks = tasks.filter(t => t.agent === 'backend' && t.tags.includes('data_access'));
                for (const backendTask of backendTasks) {
                    dependencies.push({
                        from: backendTask.id,
                        to: task.id,
     ...
     * 检测循环依赖
     */
    detectCycles(dependencies, tasks) {
        const graph = new Map();
        // 构建邻接表
        for (const task of tasks) {
            graph.set(task.id, []);
...
class DAGGenerator {
     ...
     * 寻找关键路径
     */
    findCriticalPath(nodes, topologicalOrder) {
...
class RuleForgeIntegrator {
    ...
    async matchPatterns(input) {
        ...
        for (const template of this.taskTemplates) {
            const matchScore = template.keywords.filter(keyword => text.includes(keyword.toLowerCase())).length;
            if (matchScore > 0) {
    ...
    async validateTaskDependencies(tasks) {
        ...
        for (const frontendTask of frontendTasks) {
            const hasBackendSupport = backendTasks.some(backendTask => backendTask.name.includes(frontendTask.name.replace('页面', 'API').replace('Page', 'API')));
            if (!hasBackendSupport) {
                warnings.push(`前端任务 "${frontendTask.name}" 缺少对应的后端 API 支持`);
            }
        }
        // 检查关键任务是否被标记为高优先级
        const criticalTasks = tasks.filter(t => t.tags.includes('authentication') || t.tags.includes('core_functionality'));
...
class ConversationManager {
    ...
    updateSession(sessionId, input, tasks) {
    ...
    generateClarificationQuestions(input, tasks) {
        const questions = [];
        const text = input.text.toLowerCase();
        // 检查技术栈是否明确
...
export class TaskDecomposer {
     ...
     * 主分解方法
     */
    async decompose(input, sessionId) {
        // 关键词分析
     ...
     * 从模板生成具体任务
     */
    generateTasksFromTemplates(templates, analysis) {
        const tasks = [];
     ...
     * 获取澄清问题
     */
    async getClarificationQuestions(sessionId) {
