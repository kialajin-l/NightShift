...
export function scoreManifest(
  ...
  const aliasesLower = entry.aliases.map((a: string) => a.toLowerCase());
  ...
  for (const kw of keywords) {
    ...
    if (tagsLower.some((t: string) => t.includes(kw))) {
      score += 8;
      if (8 > bestWeight) { bestWeight = 8; source = 'tag'; }
    }
    if (aliasesLower.some((a: string) => a.includes(kw))) {
      score += 7;
    }
    if (headingsLower.some((h: string) => h.includes(kw))) {
      score += 5;
...
export function updateHotset(dir: string, accessedPaths: string[]): void {
  ...
  for (const p of accessedPaths) {
    const existing = hotset.frequent.find((f: any) => f.path === p);
    if (existing) {
      existing.count += 1;
  ...
  hotset.frequent.sort((a: any, b: any) => b.count - a.count);
