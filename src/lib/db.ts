...
import fs from 'fs';
import os from 'os';
import type { ChatSession, Message, TaskItem, TaskStatus, ApiProvider, ScheduledTask } from '@/types';
import type { BatchConfig, CustomCliTool, SettingsMap, CreateProviderRequest, UpdateProviderRequest, MediaJob, MediaJobStatus, MediaJobItem, MediaJobItemStatus, MediaContextEvent } from '../types-codepilot';
 ...
 */
export function getProviderOptions(providerId: string): import('../types-codepilot').ProviderOptions {
 ...
 */
export function setProviderOptions(providerId: string, options: import('../types-codepilot').ProviderOptions): void {
  if (providerId === '__global__') {
...
export function getModelsForProvider(providerId: string): import('../types-codepilot').ProviderModel[] {
  const db = getDb();
  return db.prepare(
    'SELECT * FROM provider_models WHERE provider_id = ? AND enabled = 1 ORDER BY sort_order ASC, created_at ASC'
  ).all(providerId) as import('../types-codepilot').ProviderModel[];
}
