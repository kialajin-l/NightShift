...
export class TelegramAdapter extends BaseChannelAdapter {
   ...
   */
  override acknowledgeUpdate(updateId: number): void {
  ...
  override getPreviewCapabilities(chatId: string): PreviewCapabilities | null {
    // Global kill switch
  ...
  override async sendPreview(chatId: string, text: string, draftId: number): Promise<'sent' | 'skip' | 'degrade'> {
    const token = this.botToken;
  ...
  override endPreview(_chatId: string, _draftId: number): void {
    // No-op: the final sendMessage naturally replaces the draft
  }

  // ── Lifecycle hooks (called generically by bridge-manager) ───

  override onMessageStart(chatId: string): void {
    this.startTyping(chatId);
  }

  override onMessageEnd(chatId: string): void {
