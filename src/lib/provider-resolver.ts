...
import type { ApiProvider } from '../types-codepilot';
import {
  type Protocol,
  type AuthStyle,
  type CatalogModel,
...
export function resolveProvider(opts: ResolveOptions = {}): ResolvedProvider {
  ...
  if (effectiveProviderId && effectiveProviderId !== 'env') {
    ...
    if (provider && !(provider as any).is_active && !isExplicitRequest) {
...
function buildResolution(
  ...
  if (!roleModels.default && !roleModels.sonnet) {
    const preset = findPresetForLegacy(provider.base_url || provider.baseUrl || '', provider.provider_type || provider.type || '', protocol);
  ...
  let availableModels = getDefaultModelsForProvider(protocol, provider.base_url || provider.baseUrl || '', provider.provider_type || provider.type || '');
  try {
...
function inferAuthStyleFromProvider(provider: ApiProvider): AuthStyle {
  ...
  return inferAuthStyleFromLegacy(provider.provider_type || provider.type || '', provider.extra_env || '');
}
...
export function resolveAuxiliaryModel(
  ...
  if (main.provider) {
    const preset = findPresetForLegacy(
      main.provider.base_url || main.provider.baseUrl || '',
      main.provider.provider_type || main.provider.type || '',
      main.protocol,
    );
  ...
  if (main.provider) {
    try {
      ...
      for (const p of allProviders) {
        ...
        const protocol = getEffectiveProviderProtocol((p.type || '') as string, ((p as any).protocol || '') as string, (p.baseUrl || '') as string);
        const preset = findPresetForLegacy((p.baseUrl || '') as string, (p.type || '') as string, protocol);
        others.push({
          id: p.id,
...
export function computeEffectiveRoleModels(
  ...
  let roleModels = safeParseRoleModels((provider as any).role_models_json);
