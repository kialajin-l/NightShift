...
async function runProviderProbe(): Promise<ProbeResult> {
  ...
  for (const p of providers) {
    ...
    const protocol: Protocol = getEffectiveProviderProtocol(
      (p.provider_type || p.type || '') as string,
      (p.protocol || '') as string,
      (p.base_url || p.baseUrl || '') as string,
    );
    ...
    if (!hasModels) {
      const catalogModels = getDefaultModelsForProvider(protocol, (p.base_url || p.baseUrl || '') as string, (p.provider_type || p.type || '') as string);
    ...
    if (
      ...
      const matchedPreset = findPresetForLegacy((p.base_url || p.baseUrl || '') as string, (p.provider_type || p.type || '') as string, protocol as Protocol);
      ...
      for (const action of REPAIR_ACTIONS) {
        ...
        switch (action.type) {
          ...
          case 'switch-auth-style': {
            ...
            if (targetProvider) {
              const protocol = (targetProvider.protocol || inferProtocolFromLegacy((targetProvider.provider_type || targetProvider.type || '') as string, (targetProvider.base_url || targetProvider.baseUrl || '') as string)) as Protocol;
              const preset = findPresetForLegacy((targetProvider.base_url || targetProvider.baseUrl || '') as string, (targetProvider.provider_type || targetProvider.type || '') as string, protocol);
