          ...
          try {
            ...
            if (rawPkgArg) {
              const matchingCatalog = CLI_TOOLS_CATALOG.find(c =>
                c.installMethods.some((m: any) => m.command.includes(rawPkgArg!))
              );
              if (matchingCatalog) {
            ...
            try {
              ...
              for (const pkg of outdated) {
                const catalogTool = CLI_TOOLS_CATALOG.find(c =>
                  c.installMethods.some((m: any) => m.method === 'brew') &&
            ...
            try {
              ...
              if (stdout.trim()) {
                ...
                for (const [pkg, info] of Object.entries(outdated)) {
                  const catalogTool = CLI_TOOLS_CATALOG.find(c =>
                    c.installMethods.some((m: any) => m.method === 'npm' && m.command.includes(pkg))
                  );
          ...
          try {
            ...
            if (customTool && customTool.installMethod !== 'unknown') {
              ...
              const catalogTool = CLI_TOOLS_CATALOG.find(c =>
                ...
                c.binNames.some((b: string) => b === toolName)
              );
