  ...
  if (entryPoint === 'desktop' && session.working_directory) {
    try {
      ...
      if (config.widgets.length > 0) {
        const summary = config.widgets.map((w: any, i: number) => `${i + 1}. ${w.title} — ${w.dataContract}`).join('\n');
...
function buildBuddyPersonalityPrompt(buddy: {
  ...
  const species = buddy.species ? (SPECIES_LABEL_ZH[buddy.species] || buddy.species) : '';
  const peakHint = buddy.peakStat ? (PERSONALITY_ZH[buddy.peakStat] || '') : '';
