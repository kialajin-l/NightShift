...
export function DashboardPanel() {
  ...
  const stableWidgets = useMemo(() => {
    const ids = widgets.map((w: any) => w.id).sort();
    return ids.map((id: string) => widgets.find((w: any) => w.id === id)!);
  }, [widgets]);

  const orderMap = useMemo(() => {
    const m = new Map<string, number>();
    widgets.forEach((w: any, i: number) => m.set(w.id, i));
    return m;
  }, [widgets]);
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                downloadBlob(blob, `${(widget.title || 'widget').replace(/[^a-zA-Z0-9\u4e00-\u9fff]/g, '_')}.png`);
              } catch (e) {
