...
export async function POST(request: NextRequest) {
  try {
    ...
    const decisionObj = typeof decision === 'string' ? JSON.parse(decision) : decision;
    if (decisionObj.behavior === 'allow') {
      result = {
        behavior: 'allow',
        updatedPermissions: decisionObj.updatedPermissions as unknown[],
        ...(decisionObj.updatedInput ? { updatedInput: decisionObj.updatedInput } : {}),
      };
    } else {
      result = {
        behavior: 'deny',
        message: decisionObj.message || 'User denied permission',
      };
    }
